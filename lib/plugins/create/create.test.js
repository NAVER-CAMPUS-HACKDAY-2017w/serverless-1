...
describe('Create', () => {
  ...
  describe('#create()', () => {
    ...
    it('should generate scaffolding for "openwhisk-java-gradle" template', () => {
      process.chdir(tmpDir);
      create.options.template = 'openwhisk-java-gradle';
      ...
      return create.create().then(() => {
        ...
        expect(dirContent).to.include(path.join('src', 'main', 'java', 'com', 'serverless', 'Handler.java'));
        expect(dirContent).to.include(path.join('src', 'test', 'java', 'com', 'serverless', 'HandlerTest.java'));
      });
    });
